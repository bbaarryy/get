...
bool resize(subvector *qv, unsigned int new_capacity){
    ...
    int* curr_info = qv->mas;
    for(auto i = 0 ; i < std::min(new_capacity, qv->capacity);i++){
    ...
    qv->top = std::min(qv->top,qv->capacity);
    return 0;
} 
...
bool push_back(subvector *qv, int d){
    if(qv->capacity == qv->top){
        bool miss = resize(qv,qv->capacity + 100);
    }
